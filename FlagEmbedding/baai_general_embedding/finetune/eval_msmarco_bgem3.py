...
def index(model: BGEM3FlagModel, corpus: datasets.Dataset, batch_size: int = 256, max_length: int=512, index_factory: str = "Flat", save_path: str = None, save_embedding: bool = False, load_embedding: bool = False):
    ...
    """
    if load_embedding:
        test = model.encode("test")["dense_vecs"]
        dtype = test.dtype
        dim = len(test)
        ...
        corpus_embeddings = model.encode(corpus["content"], batch_size=batch_size, max_length=max_length)["dense_vecs"]
...
def search(model: BGEM3FlagModel, queries: datasets, faiss_index: faiss.Index, k:int = 100, batch_size: int = 256, max_length: int=512):
    """
    1. Encode queries into dense embeddings;
    2. Search through faiss index
    """
    query_embeddings = model.encode(queries["query"], batch_size=batch_size, max_length=max_length)["dense_vecs"]
